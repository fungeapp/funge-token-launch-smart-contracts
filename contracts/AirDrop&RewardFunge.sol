...
contract AirdropRewardFunge is Ownable, Initializable {
    ...
    uint256 public rewardRate = 4; // default 4% reward
     ...
     */
    function initialize(ERC20 _funge) public initializer {
        funge = _funge;
    }
    ...
    */
    ...
    // Add whitelist wallet addresses by owner
    function addWhiteListAddress(Receiver[] calldata receivers) public onlyOwner {
        uint256 receiverCount = receivers.length;
        for(uint8 i = 0; i < receiverCount; i++)
        {
            address receiver = receivers[i].wallet;
            whiteListAddress.push(receiver);
        }
    }
    ...
    function safeRewards(address _to, uint256 _amount) internal {
        ...
        if (calcRewardAmount > rewardTokenBal) {
            ...
            funge.transfer(_to, calcRewardAmount);
        }
    }
    ...
    // update funge
    function updatefunge(ERC20 _funge) public onlyOwner{
        funge = _funge;
    }

    // Add users who can claim airdrop
    function addUsersData(Receiver[] calldata _users) external onlyOwner {
